            ...
            <div class="card-header">
                ...
                <div class="d-flex flex-row-reverse"><button
                        class="btn btn-sm btn-pill btn-outline-primary font-weight-bolder" id="createNewUser"><i
                            class="fas fa-plus"></i><a href="{{route('students_form')}}">add data </a></button></div>
            </div>
            <div class="card-body">
                <div class="col-md-12">
                    <div class="table-responsive">
                        <table class="table" id="tableStudent">
                            ...
                            <tbody class="text-center">
                                {{-- @foreach ($students as $r_student)
                                    <tr>
                                <td>{{$r_student->id}}</td>
                                <td>{{$r_student->name}}</td>
                                <td>{{$r_student->fathername}}</td>
                                <td>{{$r_student->mothername}}</td>
                                <td>{{$r_student->class}}</td>
                                <td>{{$r_student->address}}</td>
                                <td>{{$r_student->mob no.}}</td>
                                <td>{{$r_student->level}}</td>
                                <td>
                                    <div class="btn btn-success editUser" data-id="{{$r_student->id}}"><a href="{{route('student_edit')}}">Edit</a></div>
                                    <div class="btn btn-danger deleteUser" data-id="{{$r_student->id}}">Delete</div>
                                </td>
                                </tr>
                                @endforeach --}}
                            </tbody>
        ...
        $('body').on('click', '.deleteUser', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: "{{route('students_data')}}" + '/' + student_id,
                        success: function (data) {
                            swal_success();
                            table.draw();
                        },
